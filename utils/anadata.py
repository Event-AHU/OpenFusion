...
def deal_dataA(domain,size):
    ...
    indices = np.random.choice(total_points, size=size, replace=False)
    x = x[indices]
    y = y[indices]
    z = z[indices]
    t = t[indices]
...
def sample_A():
    fact = 1000
    Ax_CuCrZr,Ay_CuCrZr,Az_CuCrZr,At_CuCrZr,T_CuCrZr = deal_dataA(CuCrZr,size = 20)
    Ax_Cu,Ay_Cu,Az_Cu,At_Cu,T_Cu = deal_dataA(Cu,size = 30)
    Ax_W,Ay_W,Az_W,At_W,T_W = deal_dataA(W,size = 50)
    ...
    np.save('data/Adata.npy', data_to_save)
    #print("重新采样")
#sample_A()