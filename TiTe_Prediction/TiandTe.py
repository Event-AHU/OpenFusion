...
def interpolate_profile(data, target_grid):
    ...
    _, unique_indices = np.unique(rho, return_index=True)
    rho = rho[unique_indices]
    val = val[unique_indices]

    
    core_mask_initial = rho < 0.4
    if np.sum(core_mask_initial) >= 2:
        rho_core_initial = rho[core_mask_initial]
        val_core_initial = val[core_mask_initial]
        
        
        rho_mid_core = (rho_core_initial[:-1] + rho_core_initial[1:]) / 2.0
        val_mid_core = (val_core_initial[:-1] + val_core_initial[1:]) / 2.0
        
        
        rho = np.concatenate([rho, rho_mid_core])
        val = np.concatenate([val, val_mid_core])
        
        # 重新排序
        sort_idx_core = np.argsort(rho)
        rho = rho[sort_idx_core]
        val = val[sort_idx_core]

    
    while True:
        edge_mask_initial = (rho >= 0.8) & (rho <= 1.0)
        num_edge_points = np.sum(edge_mask_initial)
        
        if num_edge_points < 2 or num_edge_points >= 5:
            break
            
        rho_edge_initial = rho[edge_mask_initial]
        val_edge_initial = val[edge_mask_initial]
        
        rho_mid_edge = (rho_edge_initial[:-1] + rho_edge_initial[1:]) / 2.0
        val_mid_edge = (val_edge_initial[:-1] + val_edge_initial[1:]) / 2.0
        
        rho = np.concatenate([rho, rho_mid_edge])
        val = np.concatenate([val, val_mid_edge])
        
        sort_idx_edge = np.argsort(rho)
        rho = rho[sort_idx_edge]
        val = val[sort_idx_edge]

    

    try:
        success = False
        if Smoothing_spline is not None and len(rho) >= 4:
            try:
                
                mask_core = rho < 0.5
                mask_edge = rho >= 0.3
                
                rho_core, val_core = rho[mask_core], val[mask_core]
                rho_edge, val_edge = rho[mask_edge], val[mask_edge]
                
                
                if len(rho_core) >= 4 and len(rho_edge) >= 4:
                    w_core = np.ones_like(rho_core)
                    
                    w_core[rho_core < 0.4] = 10.0
                    
                    
                    if rho_core[0] == 0.0:
                        w_core[0] = 1e6
                        
                    spline_core = Smoothing_spline(x=rho_core, y=val_core, w=w_core, lamda=0.01)
                    spline_core.fit()
                    
                    w_edge = np.ones_like(rho_edge)
                    
                    w_edge[rho_edge >= 0.8] = 10.0
                    
                    spline_edge = Smoothing_spline(x=rho_edge, y=val_edge, w=w_edge, lamda=0.01)
                    spline_edge.fit()
                    
                    
                    new_val = np.zeros_like(target_grid)
                    
                    
                    mask_c = target_grid < 0.30
                    mask_e = target_grid > 0.50
                    mask_m = (~mask_c) & (~mask_e)
                    
                    if np.any(mask_c):
                        new_val[mask_c] = spline_core.eval(target_grid[mask_c])
                    if np.any(mask_e):
                        new_val[mask_e] = spline_edge.eval(target_grid[mask_e])
                    if np.any(mask_m):
                        grid_m = target_grid[mask_m]
                        val_c = spline_core.eval(grid_m)
                        val_e = spline_edge.eval(grid_m)
                        
                       
                        t = (grid_m - 0.30) / 0.20
                        
                        weight_edge = (1.0 - np.cos(t * np.pi)) / 2.0
                        weight_core = 1.0 - weight_edge
                        
                        new_val[mask_m] = weight_core * val_c + weight_edge * val_e
                        
                    success = True
                else:
                    
                    w_all = np.ones_like(rho)
                    if rho[0] == 0.0:
                        w_all[0] = 1e6
                        
                    spline = Smoothing_spline(x=rho, y=val, w=w_all, lamda=0.01)
                    spline.fit()
                    new_val = spline.eval(target_grid)
                    success = True
            except Exception:
                success = False
        ...
        if not success:
            ...
            if len(rho) < 2: return np.zeros_like(target_grid)
            kind = 'cubic' if len(rho) >= 4 else ('linear' if len(rho) >= 2 else 'nearest')
            
            f = interp1d(rho, val, kind=kind, bounds_error=False, fill_value=(val[0], val[-1]))
            new_val = f(target_grid)
            
            
            new_val = np.nan_to_num(new_val, nan=0.0)
            
            
            if len(new_val) >= 5:
                new_val = savgol_filter(new_val, window_length=5, polyorder=2)
        ...
        for _ in range(2): 
            for i in range(1, len(new_val) - 1):
                if new_val[i] >= new_val[i-1]:
                    
                    avg_val = (new_val[i-1] + new_val[i+1]) / 2.0
                    new_val[i] = min(avg_val, new_val[i-1])
            
            
            if new_val[-1] >= new_val[-2]:
                new_val[-1] = new_val[-2] - 1.0 
        ...
        if rho[0] == 0.0 and target_grid[0] == 0.0:
            new_val[0] = val[0]

        
        if np.std(new_val) < 1.0 and np.std(val) > 10.0:
            
            raise ValueError("Interpolation resulted in a constant flat line despite varying input data.")

        return new_val
    except Exception as e:
        print(f"Warning: Interpolation failed or flatlined: {e}. Falling back to basic linear interpolation.")
        
        if len(rho) >= 2:
            f = interp1d(rho, val, kind='linear', bounds_error=False, fill_value=(val[0], val[-1]))
            new_val = f(target_grid)
            new_val = np.nan_to_num(new_val, nan=0.0)
            new_val[new_val < 0] = 0.0
            return new_val
