...
def load_components_from_pretrained(model, state_dict, components=['blocks','time_agg']):
    """
    :model: the model
...
def load_3d_components_from_2d(model, state_dict, components='all'):
    """
    从2D预训练权重加载到3D模型，参考dpot3d.py实现
    :model: 3D模型
    :state_dict: 2D模型的state_dict
    :components: 'all' 或组件列表 ['blocks', 'time_agg', 'patch_embed', 'pos', 'cls_head', 'out']
    """
    ...
    # 处理'all'情况
    if components == 'all':
        components = ['blocks', 'time_agg', 'patch_embed', 'pos', 'cls_head', 'out']
    
    print(f"Loading components: {components}")
    
    # 处理特定组件
    for name in components:
        if name == 'blocks' and hasattr(model, 'blocks'):
            print(f"Loading blocks from 2D to 3D...")
            for i, block in enumerate(model.blocks):
                print(f"  Loading block {i}...")
                block_state_dict = OrderedDict({k.replace(f'blocks.{i}.', ''): v for k, v in state_dict.items() if
                                                k.startswith(f'blocks.{i}.')})
                
                # 转换Conv2D权重到Conv3D: [out, in, h, w] -> [out, in, h, w, 1]
                converted_keys = []
                for k, v in block_state_dict.items():
                    if 'weight' in k and len(v.shape) == 4:
                        if any(x in k for x in ['mlp', 'proj', 'conv']):
                            block_state_dict[k] = v.unsqueeze(-1)
                            converted_keys.append(k)
                
                if converted_keys:
                    print(f"    Converted 2D->3D weights: {converted_keys}")
                
                # 只加载存在的权重，忽略不匹配的
                try:
                    block.load_state_dict(block_state_dict, strict=False)
                    print(f"    Block {i} loaded successfully")
                except Exception as e:
                    print(f"    Block {i} loading failed: {e}")
                    # 尝试逐个加载子模块
                    for sub_name, sub_module in block.named_modules():
                        if sub_name:  # 跳过空名称
                            sub_state_dict = OrderedDict({k.replace(f'{sub_name}.', ''): v for k, v in block_state_dict.items() 
                                                         if k.startswith(f'{sub_name}.')})
                            if sub_state_dict:
                                try:
                                    sub_module.load_state_dict(sub_state_dict, strict=False)
                                    print(f"      Loaded submodule: {sub_name}")
                                except:
                                    pass

        elif name == 'time_agg' and hasattr(model, 'time_agg_layer'):
            print(f"Loading time_agg_layer...")
            time_agg_state_dict = OrderedDict({k.replace('time_agg_layer.', ''): v for k, v in state_dict.items() if
                                             k.startswith('time_agg_layer.')})
            if time_agg_state_dict:
                try:
                    model.time_agg_layer.load_state_dict(time_agg_state_dict, strict=False)
                    print(f"  ✓ Time aggregation layer loaded successfully")
                except Exception as e:
                    print(f"  ✗ Time aggregation layer failed: {e}")
            else:
                print(f"  No time_agg_layer weights found")

        elif name == 'patch_embed' and hasattr(model, 'patch_embed'):
            patch_state_dict = OrderedDict({k.replace('patch_embed.', ''): v for k, v in state_dict.items() if
                                           k.startswith('patch_embed.')})
            # 转换Conv2D权重到Conv3D: [out, in, h, w] -> [out, in, h, w, 1]
            for k, v in patch_state_dict.items():
                if 'proj' in k and 'weight' in k and len(v.shape) == 4:
                    patch_state_dict[k] = v.unsqueeze(-1)
            model.patch_embed.load_state_dict(patch_state_dict, strict=False)

        elif name == 'pos' and hasattr(model, 'pos_embed'):
            # 3D位置编码需要扩展维度
            if 'pos_embed' in state_dict:
                pos_2d = state_dict['pos_embed']  # [1, C, H, W]
                if len(pos_2d.shape) == 4:  # 2D pos_embed
                    # 扩展为3D: [1, C, H, W] -> [1, C, H, W, 1]
                    pos_3d = pos_2d.unsqueeze(-1)
                    model.pos_embed = nn.Parameter(pos_3d)
                else:
                    model.pos_embed = nn.Parameter(pos_2d)

        elif name == 'cls_head' and hasattr(model, 'cls_head'):
            # 分类头直接加载，不需要转换
            model.cls_head.load_state_dict(OrderedDict(
                {k.replace('cls_head.', ''): v for k, v in state_dict.items() if
                 k.startswith('cls_head.')}), strict=False)

        elif name == 'out' and hasattr(model, 'out_layer'):
            out_state_dict = OrderedDict({k.replace('out_layer.', ''): v for k, v in state_dict.items() if
                                         k.startswith('out_layer.')})
            # 转换ConvTranspose2D到ConvTranspose3D权重
            for k, v in out_state_dict.items():
                if 'weight' in k and len(v.shape) == 4:
                    out_state_dict[k] = v.unsqueeze(-1)
            model.out_layer.load_state_dict(out_state_dict, strict=False)
