...
import torch
import numpy as np
from utils.visualize_predictions_3d import visualize_sample, visualize_3d_volume
...
parser.add_argument('--model', type=str, default='DPOT_MoE_3D')  # 3D模型
...
parser.add_argument('--slice_mode', type=str, default='xy', choices=['xy', 'xz', 'yz', 'all'])
parser.add_argument('--slice_idx', type=int, default=None)
parser.add_argument('--visualize_3d_volume', action='store_true')
parser.add_argument('--volume_slice_indices', type=str, default=None)
...
parser.add_argument('--out_layer_dim', type=int, default=16)  
...
train_dataset = TemporalDataset3D(args.train_paths, args.ntrain_list, res=args.res, t_in=args.T_in, t_ar=args.T_ar, normalize=False, train=True, data_weights=args.data_weights, n_channels=args.n_channels)
test_datasets = [TemporalDataset3D(test_path, [args.ntest_list[i]], res=args.res, n_channels=train_dataset.n_channels, t_in=args.T_in, t_ar=-1, normalize=False, train=False) for i, test_path in enumerate(test_paths)]
    ...
    for id, test_loader in enumerate(test_loaders):
        ...
        for batch_idx, (xx, yy, msk, _) in enumerate(test_loader):
            ...
            for t in range(0, yy.shape[-2], args.T_bundle):
                ...
                sample_id = batch_idx * args.batch_size  
                visualize_sample(im, y, sample_id, test_paths[id], 
                               sample_idx=0, 
            ...
            if args.visualize_3d_volume:
                volume_slice_indices = None
                if args.volume_slice_indices:
                    volume_slice_indices = {}
                    for slice_info in args.volume_slice_indices.split(';'):
                        if ':' in slice_info:
                            slice_type, indices_str = slice_info.split(':')
                            indices = [int(x.strip()) for x in indices_str.split(',')]
                            volume_slice_indices[slice_type] = indices
                
                for channel_idx in range(pred.shape[-1]):
                    visualize_3d_volume(pred, yy, sample_id, test_paths[id], 
                                      sample_idx=0, 
                                      channel_idx=channel_idx,
                                      slice_indices=volume_slice_indices)
