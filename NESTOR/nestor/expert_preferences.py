...
import torch
import numpy as np
from utils.visualize_predictions import visualize_sample
...
parser.add_argument('--model', type=str, default='DPOT')  # AFNO
...
parser.add_argument('--resume_path',type=str, default='/media/data4/zxy/DPOT/DPOT-main/logs_pretrain/DPOT_ns2d_0513_18_13_35:Ti_12_129288/model_0.pth')
...
test_loaders = [
    torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=8)
    for test_dataset in test_datasets
]

ntests = args.ntest_list
...
with torch.no_grad():
    ...
    for dataset_id, test_loader in enumerate(test_loaders):
        dataset_name = test_paths[dataset_id]
        ...
        # 每个数据集的 MoE 统计
        moe1_expert_total = None
        moe2_expert_total = None
        total_samples = 0
        ...
        for xx, yy, msk, _ in test_loader:
            ...
            for t in range(0, yy.shape[-2], args.T_bundle):
                ...
                loss += myloss(im, y, mask=msk) + moe_aux1 * 1e-2 + moe_aux2 * 1e-5
                ...
                # MoE1
                if moe1_expert_total is None:
                    moe1_expert_total = batch_expert_counts.detach().cpu().clone()
                else:
                    moe1_expert_total += batch_expert_counts.detach().cpu()

                # MoE2
                if moe2_expert_total is None:
                    moe2_expert_total = moe2_batch_expert_counts.detach().cpu().clone()
                else:
                    moe2_expert_total += moe2_batch_expert_counts.detach().cpu()

                # 只在最后一个时间片保存
                # if t == yy.shape[-2] - args.T_bundle:
                #     visualize_sample(im, y, global_sample_idx, dataset_name, sample_idx=0)
                #     global_sample_idx += 1
        ...
        # ========= 本数据集的 L2 误差统计 =========
        ntest = ntests[dataset_id]
        test_l2_step_avg = test_l2_step / ntest / (T_total / args.T_bundle)
        test_l2_full_avg = test_l2_full / ntest

        test_l2_steps.append(test_l2_step_avg)
        test_l2_fulls.append(test_l2_full_avg.item())

        print(f"[{dataset_name}] L2_step_avg: {test_l2_step_avg:.6f}, "
              f"L2_full_avg: {test_l2_full_avg.item():.6f}")

        # ========= 本数据集的 MoE 专家偏好 =========
        if moe1_expert_total is not None:
            moe1_prob = moe1_expert_total / (moe1_expert_total.sum() + 1e-8)
            moe2_prob = moe2_expert_total / (moe2_expert_total.sum() + 1e-8)

            print("\n--- MoE expert usage statistics ---")
            print(f"Total samples: {total_samples}")

            print("MoE1 专家总次数: ", moe1_expert_total.tolist())
            print("MoE1 专家使用比例: ", [float(x) for x in moe1_prob.tolist()])
            print("MoE1 最偏好专家 index: ", int(moe1_prob.argmax()))
