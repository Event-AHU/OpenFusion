import sys
import os
os.environ['ACCELERATE_FIND_UNUSED_PARAMETERS'] = 'true' 
sys.path.append(['.','./../'])
...
from models.nestor import Nestor
...
if __name__ == "__main__":
    ...
    # accelerator = Accelerator(split_batches=False,find_unused_parameters=True)  # 加了后面
    
    ddp_kwargs = DistributedDataParallelKwargs(find_unused_parameters=True)
    accelerator = Accelerator(kwargs_handlers=[ddp_kwargs], split_batches=False)  # 这俩
    ...
    log_path = './Scaling/' + time.strftime('%m%d_%H_%M_%S') + comment if len(args.log_path)==0  else os.path.join('./Scaling',args.log_path + comment)
...
# # ===== checkpoint =====
#     checkpoint_path = "/zxy/dpot/Scaling/DPOT_MoE_ns2d_1112_20_38_47:6num_6_70000/model_4.pth"
#     try:
    ...
    for ep in range(args.epochs):
        ...
        for xx, yy, msk, cls in train_loader:
            ...
            for t in range(0, yy.shape[-2], args.T_bundle):
                ...
                # 获取专家激活计数
                im, cls_pred, moe_aux1, moe_aux2, batch_expert_counts, moe2_batch_expert_counts = model(xx)
                loss += myloss(im, y, mask=msk) + moe_aux1 * 3e-2 + moe_aux2 * 1e-4

                # 累加专家激活计数
                if epoch_expert_counts is None:
                    epoch_expert_counts = batch_expert_counts.detach().cpu().clone()
                else:
                    epoch_expert_counts += batch_expert_counts.detach().cpu()

                # 统计MoE2专家激活计数
                if 'epoch_moe2_expert_counts' not in locals() or epoch_moe2_expert_counts is None:
                    epoch_moe2_expert_counts = moe2_batch_expert_counts.detach().cpu().clone()
                else:
                    epoch_moe2_expert_counts += moe2_batch_expert_counts.detach().cpu()
        ...
        # 每轮结束后写入专家激活频率百分比到log文件
        if epoch_expert_counts is not None:
            total = epoch_expert_counts.sum().item()
            if total > 0:
                freq_percent = (epoch_expert_counts / total * 100).tolist()
                freq_str = '\t'.join([f"{p:.2f}%" for p in freq_percent])
            else:
                freq_str = '\t'.join(["0.00%" for _ in range(len(epoch_expert_counts))])
            # with open('moe_expert_freq.txt', 'a') as f:
            #     f.write(freq_str + '\n')
        ...
        if args.use_writer:
            torch.save({'args': args, 'model': model.state_dict(), 'optimizer': optimizer.state_dict(),'scheduler': scheduler.state_dict()}, model_path_fun(ep // ckpt_save_epochs))

        t_test = default_timer() - t_1
        t2 = t_1 = default_timer()
        lr = optimizer.param_groups[0]['lr']
        log_line = 'epoch {}, time {:.5f}, lr {:.2e}, train l2 step {:.5f} train l2 full {:.5f}, test l2 step {} test l2 full {}, cls acc {:.5f}, time train avg {:.5f} load avg {:.5f} test {:.5f}'.format(ep, t2 - t1, lr,train_l2_step_avg, train_l2_full_avg,', '.join(['{:.5f}'.format(val) for val in test_l2_steps]),', '.join(['{:.5f}'.format(val) for val in test_l2_fulls]), cls_acc, t_train / len(train_loader), t_load / len(train_loader), t_test)
        print(log_line)
        ...
        if epoch_expert_counts is not None:
            ...
            if total > 0:
                freq_percent = (epoch_expert_counts / total * 100).tolist()
                freq_str = '\t'.join([f"{p:6.2f}%" for p in freq_percent])
            else:
                freq_str = '\t'.join(["  0.00%" for _ in range(len(epoch_expert_counts))])
            print(freq_str)

        # MoE2专家激活频率
        if 'epoch_moe2_expert_counts' in locals() and epoch_moe2_expert_counts is not None:
            total2 = epoch_moe2_expert_counts.sum().item()
            if total2 > 0:
                freq_percent2 = (epoch_moe2_expert_counts / total2 * 100).tolist()
                freq_str2 = '\t'.join([f"{p:6.2f}%" for p in freq_percent2])
            else:
                freq_str2 = '\t'.join(["  0.00%" for _ in range(len(epoch_moe2_expert_counts))])
            print(freq_str2)
