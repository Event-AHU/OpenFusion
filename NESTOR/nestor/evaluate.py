...
parser.add_argument('--resume_path',type=str, default='/zxy/dpot/merge/merged_model_11.pth')
...
test_index = 0  # 比如选第一个，也可以写成 2、3 …
test_datasets = [
    MixedTemporalDataset(
        args.test_paths[test_index],
        [args.ntest_list[test_index]],   
        res=args.res,
...
log_path = './log' + time.strftime('%m%d_%H_%M_%S') + comment if len(args.log_path)==0  else os.path.join('./merge',args.log_path + comment)
