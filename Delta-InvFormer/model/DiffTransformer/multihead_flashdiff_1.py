...
from model.DiffTransformer.kernel.rotary import apply_rotary_emb
...
try:
    ...
    from model.DiffTransformer.rms_norm import RMSNorm
...
class MultiheadFlashDiff1(nn.Module):
    ...
    def forward(
        ...
        q1, q2 = q[:, :, :, 0], q[:, :, :, 1]
        k1, k2 = k[:, :, :, 0], k[:, :, :, 1]
        attn1 = F.scaled_dot_product_attention(q1, k1, v, is_causal=False)
        attn2 = F.scaled_dot_product_attention(q2, k2, v, is_causal=False)
...
class MultiheadCrossFlashDiff1(nn.Module):
    """
    (Recommended)
    DiffAttn implemented with FlashAttention, for packages that support different qk/v dimensions
    e.g., our customized flex_head_fa (https://aka.ms/flash-diff) and xformers (https://github.com/facebookresearch/xformers)
